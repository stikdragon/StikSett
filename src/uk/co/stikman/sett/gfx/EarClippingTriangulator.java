...
public class EarClippingTriangulator {
	...
	private final IntList		indicesArray	= new IntList();
	private int[]				indices;
	private float[]				vertices;
	private int					vertexCount;
	private final IntList		vertexTypes		= new IntList();
	private final IntList		triangles		= new IntList();

	/** @see #computeTriangles(float[], int, int) */
	public IntList computeTriangles(float[] vertices) {
	 ...
	 */
	public IntList computeTriangles(float[] vertices, int offset, int count) {
		this.vertices = vertices;
		int vertexCount = this.vertexCount = count / 2;

		IntList indicesArray = this.indicesArray;
		indicesArray.clear();
		indicesArray.setSize(vertexCount);
		int[] indices = this.indices = indicesArray.list;
		...
		IntList vertexTypes = this.vertexTypes;
		vertexTypes.clear();
		for (int i = 0, n = vertexCount; i < n; ++i)
			vertexTypes.add(classifyVertex(i));

		// A polygon with n vertices has a triangulation of n-2 triangles.
		IntList triangles = this.triangles;
		triangles.clear();
	...
	private void triangulate() {
		int[] vertexTypes = this.vertexTypes.list;
	...
	private void cutEarTip(int earTipIndex) {
		...
		indicesArray.remove(earTipIndex);
